...
def test_ParticleBunch__repr__():
    ...
    assert re.findall(
        "Name: test_Bunch 1", test_ParticleBunch.listOfParticles[0].__repr__()) == ["Name: test_Bunch 1"]
...
def test_CreateListOfParticles():
    ...
    assert np.std([test_ParticleBunch.listOfParticles[i].position for i in 
        range(test_ParticleBunch.numberOfParticles)]) == pytest.approx(1.0, abs=0.07)
    # check that the particles have the correct mean position
    assert np.mean([test_ParticleBunch.listOfParticles[i].position for i in 
        range(test_ParticleBunch.numberOfParticles)]) == pytest.approx(0.0, abs=0.07)
...
def test_FindBunchMeanMomentum():
    # checks that the method for finding the mean momentum of particles in the bunch is correct
    assert test_ParticleBunch.FindBunchMeanMomentum()[0] == pytest.approx(1.672621898e-24, rel=0.05)